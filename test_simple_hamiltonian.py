...
axs[1].plot(t/(np.pi*2), p_sol[:,1], label=r"$(I_1,\theta_1)$")
axs[1].plot(t/(np.pi*2), p_sol[:,0], label=r"$(I_2,\theta_2)$")
...
axs[1].plot(t/(np.pi*2), p_sol[:,1], label=r"$(I_1,\theta_1)$", c='grey', linestyle="--")
axs[1].plot(t/(np.pi*2), p_sol[:,0], label=r"$(I_2,\theta_2)$", c='black', linestyle=":")
axs[1].plot(t/(np.pi*2), J1, label=r"$(J_1,\phi_1)$")
axs[1].plot(t/(np.pi*2), J2, label=r"$(J_2,\phi_2)$")
axs[1].set_ylim(-0.2,6)
axs[1].set_xlim(0,10)
axs[1].set_ylabel("$J_n$")
axs[1].set_xlabel("$t$")
...
axs[1].plot(t/(np.pi*2), J1, label=r"$(J_1,\phi_1)$")
axs[1].plot(t/(np.pi*2), J2, label=r"$(J_2,\phi_2)$")
axs[1].plot(t/(np.pi*2), np.abs(psi_cancelled[0]), label="$X_1^{(2)}$")
axs[1].plot(t/(np.pi*2), np.abs(psi_cancelled[1]), label="$X_2^{(2)}$")
axs[1].set_ylim(-0.2,6)
axs[1].set_xlim(0,10)
axs[1].set_ylabel("$J_n$")
axs[1].set_xlabel("$t$")
...
fig, axs = plt.subplots(1,2, figsize=(8,3))
t_plot = t<10
axs[0].plot(jnp.real(complex_sol[t_plot,0]), jnp.imag(complex_sol[t_plot,0]), linewidth=0.5)
axs[0].plot(jnp.real(complex_sol[t_plot,1]), jnp.imag(complex_sol[t_plot,1]), linewidth=0.5)
axs[0].plot(jnp.real(psi_cancelled.T[t_plot,0]), jnp.imag(psi_cancelled.T[t_plot,0]), linewidth=0.5)
axs[0].plot(jnp.real(psi_cancelled.T[t_plot,1]), jnp.imag(psi_cancelled.T[t_plot,1]), linewidth=0.5)
axs[0].set_aspect('equal')
axs[0].set_xlim(-6,6)
...
plt.tight_layout()
plt.savefig("figs/simple-example-computational-poster.eps")
# %%